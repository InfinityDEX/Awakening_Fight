...
// コンストラクタ
DefaultEffect::DefaultEffect(int life, Vector3 scale, Vector3 pos, const wchar_t * path)
	: m_life(life)
	, m_scale(scale)
{
	// デバイス取得
...
// 更新処理
void DefaultEffect::Update(const DX::StepTimer & timer)
{
	// ライフが0(以下)だったら
	if (m_life <= 0)
	{
...
// 描画処理
void DefaultEffect::Render(Matrix world, Matrix view, Matrix proj)
{
	// デバイスの取得
	auto device = Locator::GetDeviceResources()->GetD3DDevice();
	// コンテキストの取得
