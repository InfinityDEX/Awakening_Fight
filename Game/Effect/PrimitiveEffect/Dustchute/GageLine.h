...
class GageLine : public IObject
{
	public:
		struct ConstBuffer
		{
			...
			DirectX::SimpleMath::Vector4	Status;
			DirectX::SimpleMath::Vector4	Color;
		};
		static const std::vector<D3D11_INPUT_ELEMENT_DESC> INPUT_LAYOUT;
		GageLine(Vector2 pos, Vector2 size, int dir, const int* value, int maxValue, Vector4 color = static_cast<Vector4>(Colors::Yellow));
		~GageLine();
		virtual void Create();
		...
		void FlashGage(float totalSeconds);
	...
	private:
		...
		const Vector3 m_defaultPosition;
		Vector3 m_vibrationVel;
		// 頂点シェーダ
