...
// シェーダ
Microsoft::WRL::ComPtr<ID3D11PixelShader> DefaultPS::m_shader;
...
DefaultPS::DefaultPS()
{
	...
	/// ジオメトリシェーダを作成する
	// ジオメトリシェーダのファイル読み込み
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/Default/DefaultPS.cso");
	// ピクセルシェーダ作成
...
// シェーダーの取得
ID3D11PixelShader * DefaultPS::GetShader()
{
	return m_shader.Get();
}

// 定数バッファの取得
DefaultPS::ConstBuffer & DefaultPS::GetCBuffer()
{
	return m_cbuff;
}
