...
// シェーダ
Microsoft::WRL::ComPtr<ID3D11PixelShader> ColorPS::m_shader;
...
ColorPS::ColorPS()
{
	...
	/// ジオメトリシェーダを作成する
	// ジオメトリシェーダのファイル読み込み
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/Color/ColorPS.cso");
	// ピクセルシェーダ作成
...
ID3D11PixelShader * ColorPS::GetShader()
{
	return m_shader.Get();
}

ColorPS::ConstBuffer & ColorPS::GetCBuffer()
{
	return m_cbuff;
}
