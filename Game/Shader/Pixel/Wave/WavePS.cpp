...
// シェーダ
Microsoft::WRL::ComPtr<ID3D11PixelShader> WavePS::m_shader;
...
WavePS::WavePS()
{
	...
	/// ピクセルシェーダを作成する
	// ピクセルシェーダのファイル読み込み
	BinaryFile PSData = BinaryFile::LoadFile(L"Resources/Shaders/Wave/WavePS.cso");
	// ピクセルシェーダ作成
...
ID3D11PixelShader * WavePS::GetShader()
{
	return m_shader.Get();
}

WavePS::ConstBuffer & WavePS::GetCBuffer()
{
	return m_cbuff;
}
